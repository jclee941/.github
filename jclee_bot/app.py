...
def _checkout_pr_head(token: str, repo_full_name: str, head_sha: str, workspace: str) -> bool:
    ...
    try:
        subprocess.run(["git", "init", "-q", workspace], check=True, timeout=30)  # noqa: S603 - fixed args
        ...
        if pull_request_ok or issue_event_ok or gitops_event_ok:
            try:
                ...
                if event == "pull_request":
                    ...
                    asyncio.get_event_loop().run_in_executor(None, _run_issue_management_for_payload, payload, event)
                elif event in {"create", "pull_request_review"}:
                    import asyncio

                    asyncio.get_event_loop().run_in_executor(None, _run_gitops_automation_for_payload, payload, event)
