...
from jclee_bot.review_engine.algo.utils import Range, process_description
...
def _wrap_publish_method(method):
    ...
    def _apply(args, kwargs):
        try:
            from jclee_bot.review_engine.algo.secret_masking import mask_obj, mask_text
        except Exception:
            return args, kwargs
        args = list(args)
...
class GitProvider(ABC):
    ...
    def get_pr_description(self, full: bool = True, split_changes_walkthrough=False) -> str | tuple:
        from jclee_bot.review_engine.algo.utils import clip_tokens
        from jclee_bot.review_engine.config_loader import get_settings
