...
from jclee_bot.review_engine.config_loader import _find_repository_root, get_settings
...
class LocalGitProvider(GitProvider):
    ...
    def get_diff_files(self) -> list[FilePatchInfo]:
        ...
        for diff_item in diffs:
            # Binary files (images, PDFs, archives) are not utf-8 decodable;
            # decode with errors='replace' so we still produce a valid diff
            # entry instead of crashing the whole review.
            if diff_item.a_blob is not None:
                original_file_content_str = diff_item.a_blob.data_stream.read().decode("utf-8", errors="replace")
            else:
            ...
            try:
                ...
                patch_text = "<binary diff omitted>"
            diff_files.append(
                FilePatchInfo(
