...
from jclee_bot.review_engine.algo import (
...
from jclee_bot.review_engine.algo.ai_handlers.litellm_helpers import (
    MockResponse,
    _get_azure_ad_token,
    _handle_streaming_response,
    _process_litellm_extra_body,
)
from jclee_bot.review_engine.algo.utils import ReasoningEffort, get_version
...
class LiteLLMAIHandler(BaseAiHandler):
    ...
    async def chat_completion(self, model: str, system: str, user: str, temperature: float = 0.2, img_path: str = None):
        from jclee_bot.review_engine.algo.secret_masking import mask_text
            ...
            if img_path:
                try:
                    ...
                    if r.status_code == 404:
                        error_msg = f"The image link is not [alive](img_path).\nPlease repost the original image as a comment, and send the question again with 'quote reply' (see [instructions](https://github.com/qodo-ai/pr-agent/blob/main/docs/docs/tools/ask.md))."
    ...
    async def _get_completion(self, **kwargs):
        """
        Wrapper that keeps all LLM traffic on the configured CLIProxyAPI base.

        Model fallback is handled by pr-agent's fallback model chain. Provider
        fallback is intentionally disabled so an unavailable CLIProxyAPI fails
        fast instead of routing requests to a different base URL.
        """
        return await self._get_completion_once(**kwargs)
    ...
    async def _get_completion_once(self, **kwargs):
        ...
        if "/" not in model and any(model.startswith(p) for p in ("kimi-", "minimax-", "MiniMax-", "claude-", "gpt-", "codex-", "gemini-")):
