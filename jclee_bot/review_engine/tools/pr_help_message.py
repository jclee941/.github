...
from jclee_bot.review_engine.algo import MAX_TOKENS
...
from jclee_bot.review_engine.algo.pr_processing import retry_with_fallback_models
from jclee_bot.review_engine.algo.token_handler import TokenHandler
from jclee_bot.review_engine.algo.utils import ModelType, clip_tokens, get_max_tokens, load_yaml
from jclee_bot.review_engine.config_loader import get_settings
from jclee_bot.review_engine.git_providers import BitbucketServerProvider, GithubProvider, get_git_provider_with_context
    ...
    async def run(self):
        try:
            if self.question_str:
                ...
                base_path = "https://github.com/qodo-ai/pr-agent/blob/main/docs/docs/tools"
                ...
                if isinstance(self.git_provider, GithubProvider) and not get_settings().config.get('disable_checkboxes', False):
                    ...
                    pr_comment += "</table>\n\n"
                    pr_comment += f"""\n\n(1) Note that each tool can be [triggered automatically](https://github.com/qodo-ai/pr-agent/blob/main/docs/docs/usage-guide/automations_and_usage.md) when a new PR is opened, or called manually by [commenting on a PR](https://github.com/qodo-ai/pr-agent/blob/main/docs/docs/usage-guide/automations_and_usage.md)."""
                    ...
                    pr_comment += "</table>\n\n"
                    pr_comment += f"""\n\nNote that each tool can be [invoked automatically](https://github.com/qodo-ai/pr-agent/blob/main/docs/docs/usage-guide/automations_and_usage.md) when a new PR is opened, or called manually by [commenting on a PR](https://github.com/qodo-ai/pr-agent/blob/main/docs/docs/usage-guide/automations_and_usage.md)."""
