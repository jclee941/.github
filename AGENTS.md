...
Source-of-truth automation repo for `jclee941/*` PR flow. It combines the `jclee_bot` GitHub App checks runner, the absorbed review engine (`jclee_bot.review_engine`, originally derived from `qodo-ai/pr-agent`), Go/Python automation scripts, GitHub Actions workflows, and downstream community-file templates.
...
| `jclee_bot.app._tee_pull_request_to_checks` | middleware | `jclee_bot/app.py` | Tees the review-engine webhook into App checks |
