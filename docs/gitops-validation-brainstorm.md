...
| C2 PR 리뷰 | `10_pr-review`(CLIProxy), `security/11_pr-review`(보안 감사), `86_pr-review-security` | `86`은 `security/11`을 복제한 미사용 reusable | 🔴 `86` 제거 |
| C3 Stale | `16_stale-repo-identifier`, `17_pr-stale-bot`, `88_stale` | `88`은 `16`/`17`을 복제한 미사용 reusable | 🔴 `88` 제거 |
| C4 이슈 관리 | `18_issue-management`, `19_issue-backfill`, `91_issue-classification`, `82_issue-label`, `83_issue-lifecycle`, `84_labeler` | `82`/`83`/`84`는 `18`/`91`을 복제한 미사용 reusable | 🔴 `82`/`83`/`84` 제거 |
...
> 비교군: `42_reusable-docs-sync`(←`20`/`21`), `43_reusable-issue-management`(←`18`), `44_reusable-pr-checks`(←`03`), `45_reusable-gitleaks`(←`05`) 는 실제 호출되므로 유지. `security/11_pr-review` 는 `pull_request_target` 자체 트리거 + 다운스트림 배포 대상이므로 유지.
