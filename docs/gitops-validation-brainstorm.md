...
> 비교군: `42_reusable-docs-sync`(←`20`/`21`), `43_reusable-issue-management`(←`18`), `44_reusable-pr-checks`(←`03`), `45_reusable-gitleaks`(←`05`) 는 실제 호출되므로 유지. `11_security-pr-review` 는 `pull_request_target` 자체 트리거 + 다운스트림 배포 대상이므로 유지.
