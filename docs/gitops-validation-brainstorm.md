...
**분석일**: 2026-06-03
**분석 + 구현**: 동일 PR
**상태**: historical brainstorming record. Current implementation guidance lives in `docs/git-automation-masterplan.md`.
**분석 범위(당시 PR 기준)**: `.github/workflows/` (57개 → 47개), `scripts/cmd/*` (Go 도구 8개), `scripts/*.py`, `scripts/cmd/deploy-to-repos/main.go` 배포 매니페스트

> 본 문서는 2026-06-03 시점의 "분석(브레인스토밍)"과 "구현(중복 제거 + 검증 도구 추가)"을 한 PR로 다룬 historical record입니다.
> 현재 git 자동화 운영 계획, 검증 순서, live evidence 기준은 `docs/git-automation-masterplan.md`를 따릅니다.
> 2026-06-30 현재 `.github/workflows/`에는 26개 workflow 파일이 있습니다.
>
> - **As-was**: 이 PR 직전의 상태(2026-06-03)
> - **As-is**: 이 PR 적용 직후의 상태(2026-06-03)
...
| 항목 | As-was | As-is |
|---|---|---|
| 활성/존재 워크플로 수(2026-06-03 PR 기준) | 57 | **47** |
...
핵심 결론(2026-06-03 PR 기준): **57개 워크플로 중 10개는 `on: workflow_call` 전용이면서 로컬에서 아무도 호출하지 않고, 다운스트림 배포 매니페스트(`downstreamWorkflowAllowlist`)에도 없는 "고아 재사용 워크플로"** 였다. 이들은 활성 워크플로(12/13/17/18/91 등)의 기능을 중복하는 dead code였고, 인벤토리 정확도를 떨어뜨리며 GitOps 드리프트의 원인이 되었다. 전부 제거하고, **재발 방지용 자동 검증기**(`orphanReusableWorkflows`)를 추가했다.
...
2026-06-03 PR 당시 57개 워크플로를 트리거(`on:`)와 역할 기준으로 군집화한 결과, 다음 9개 클러스터에서 기능 중복이 관찰되었다.
...
→ 2026-06-03 PR 기준 57 → 47개. 당시 README 인벤토리(badge/TOC/표/디렉터리 트리) 동기화 완료.
...
- [x] 10개 고아 워크플로 제거 (2026-06-03 PR 기준 57 → 47)
...
- [x] README 끌글링 참조 0개, 당시 카운트 47 동기화, 카테고리 소계 일치, markdownlint 0 error
