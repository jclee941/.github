...
def product_readme_system_prompt() -> str:
    return "".join(
        (
            ...
            "Do NOT brand the repository as a monorepo in the title or one-line summary just because ",
            "the tree contains apps/, packages/, workspaces, or multiple services. Treat monorepo as a ",
            "technical structure detail, not the product identity. If an existing README uses stale ",
            "repository-name-plus-monorepo branding, replace it with a user-facing product/workspace ",
            "name derived from actual repository purpose and metadata. ",
